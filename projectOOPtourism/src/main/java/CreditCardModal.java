...
public class CreditCardModal { // No longer extends AnchorPane
    ...
    private void formatCardNumber(String text) {
        String unformatted = text.replaceAll("[^\\d]", "");
        ...
        // Only update if the formatted text is different to avoid infinite loop with listener
        if (!cardNumberField.getText().equals(formatted.toString())) {
            Platform.runLater(() -> cardNumberField.setText(formatted.toString()));
        }
    }
